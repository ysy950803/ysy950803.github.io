...
先从手机管家开始，因为它是桌面上就可见的系统App，把Apk提取出来（不需要Root，方法此处不赘述）进行反编译。这里用到了 **jadx-gui** 开源工具，可以直接全局搜索文本。开动你的脑筋，和5G相关的代码会怎么写呢？机智地我进行了如下关键词猜测：5G、FiveG、5thGeneration，简直是煞费苦心啊！然后，果然有：
...
可以看到在powercenter包下面有一堆反射调用，虽然有混淆代码，但是反射类名是可见的：
